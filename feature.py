...
class FeatureExtraction:
    ...
    def WebsiteTraffic(self):
        try:
            rank = BeautifulSoup(
                urllib.request.urlopen("http://data.alexa.com/data?cli=10&dat=s&url=" + self.url).read(), "xml"
            ).find("REACH")['RANK']
            if int(rank) < 100000:
                return 1
            return 0
    ...
    def getFeaturesDict(self):
        names = [
            "Using IP Address", "Long URL", "Short URL Service", "@ Symbol",
            "Double Slash Redirect", "Prefix/Suffix Hyphen", "Sub Domains",
            "HTTPS Protocol", "Domain Registration Length", "Favicon",
            "Non-Standard Port", "HTTPS in Domain URL", "Request URL",
            "Anchor URL", "Links in Script Tags", "Server Form Handler",
            "Info Email", "Abnormal URL", "Website Forwarding",
            "Status Bar Customization", "Disable Right Click", "Popup Window",
            "Iframe Redirection", "Age of Domain", "DNS Recording",
            "Website Traffic", "Page Rank", "Google Index",
            "Links Pointing to Page", "Stats Report",
        ]
        return dict(zip(names, self.features))
